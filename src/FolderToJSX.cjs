...
// Fonction pour générer le contenu par défaut avec le nom du fichier et les sous-dossiers
const genererContenu = (nomFichier, sousDossiers) => {
		const sousDossiersHTML = sousDossiers.map(sd => `<button className='CTA-notice'>${sd}</button>`).join('\n');
				...
				dossiers.forEach(dossier => {
						...
						if (fichiersExistantSet.has(nomFichierJSX)) {
								...
								fs.readdir(dossierPath, (err, sousDossiers) => {
										...
										// Générer le contenu avec le nom du fichier et les sous-dossiers
										const contenu = genererContenu(nomFichierJSX.replace('.jsx', ''), sousDossiers);
